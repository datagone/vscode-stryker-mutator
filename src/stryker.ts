...
const makeCommand = (file?: vscode.Uri, range?: string) => {
  ...
  // const target = `${path.basename(file.fsPath)}${lineRange ? `:${lineRange}` : ''}`;
  // TODO : We could add other params to (maybe from the settings ?!?) AND/OR using the BuilderPattern
  // return `${strykerBin} --mutate "${target}"${withConfigFileParams}`;
