  ...
  describe('WHEN command to mutate a solution', () => {
    ...
    describe('GIVEN there is no solution path to mutate', () => {
      ...
      describe('AND GIVEN the selection returns a valid file to mutate', () => {
        it('THEN should not execute the commandRunner', async () => {
          ...
          mockPathToMutate.prototype.addPathToMutate.mockResolvedValue();
          ...
          expect(mockPathToMutate.prototype.addPathToMutate).toHaveBeenCalledTimes(1);
    ...
    //   await mutateSolutionCommand(run)(new Uri({ path: uriPath }));
    ...
    // it('should run the command if the active document ia a .cs file', async () => {
    //   const run = jest.fn();
    //   const uriPath: string = 'x.cs';
